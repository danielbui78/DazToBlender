...
class DtbShaders:
	...
	def setup_materials(self, obj):
		...
		for mat_slot in obj.material_slots:
			...
			print ("setup_materials() original object name is ", obj_name)
			obj_name = obj_name.split(".")[0]
			print ("setup_materials() fixed object name is ", obj_name)
			if mat is None:
				print("mat is None")
				# Get or create a new material when slot is missing material
				mat = bpy.data.materials.get(mat_slot.name) or bpy.data.materials.new( name=mat_slot.name )
